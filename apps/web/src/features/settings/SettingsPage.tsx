...
import { LoadingState, OfflineState } from '../../components/ui/States.js'
...
export function SettingsPage() {
  ...
  if (isOffline) return <OfflineState title="Pengaturan membutuhkan koneksi" description="Identitas toko dan kata sandi tidak disimpan offline." />
  ...
  return (
    <div className="page-stack settings-page">
      ...
      <div className="settings-grid">
        <StoreForm settings={settings.data} canWrite={canWrite} />
        <PasswordForm canWrite={canWrite} />
      </div>
    </div>
  )
}

function StoreForm({ settings, canWrite }: { settings: StoreSettings; canWrite: boolean }) {
  ...
  const mutation = useMutation({
    mutationFn: (input: StoreSettingsInput) => {
      ...
      return apiRequest<StoreSettings>('/api/v1/settings/store', {
        method: 'PATCH',
        ...jsonBody(input),
      })
    },
  ...
  return (
    <section className="settings-panel">
      ...
      <form className="form-stack" noValidate onSubmit={form.handleSubmit((input) => mutation.mutate(input))}>
        ...
        <div className="form-actions"><Button type="submit" icon={<Save />} pending={mutation.isPending} disabled={!canWrite} disabledReason="Sambungkan koneksi untuk menyimpan pengaturan">Simpan identitas</Button></div>
      </form>
...
function PasswordForm({ canWrite }: { canWrite: boolean }) {
  const queryClient = useQueryClient()
  const form = useForm<PasswordChangeInput>({ resolver: zodResolver(passwordChangeInputSchema) })
  const mutation = useMutation({
    mutationFn: (input: PasswordChangeInput) => { assertOnlineWrite(canWrite); return apiRequest<void>('/api/v1/settings/password', { method: 'POST', ...jsonBody(input) }) },
    onSuccess: async () => {
      form.reset()
  ...
  return (
    <section className="settings-panel settings-panel--security">
      ...
      <form className="form-stack" onSubmit={form.handleSubmit((input) => mutation.mutate(input))}>
        ...
        <div className="form-actions"><Button type="submit" icon={<KeyRound />} pending={mutation.isPending} disabled={!canWrite} disabledReason="Sambungkan koneksi untuk mengubah kata sandi">Ganti kata sandi</Button></div>
      </form>
