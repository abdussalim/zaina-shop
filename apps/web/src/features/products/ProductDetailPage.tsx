...
import type { Category, Product, StockMovement } from '../../api/types.js'
...
export function ProductDetailPage() {
  ...
  const movements = useQuery({ queryKey: ['inventory', 'movements', id], queryFn: () => apiRequest<StockMovement[]>(`/api/v1/inventory/movements?productId=${id}&limit=100`), enabled: Boolean(id) })
    ...
    <div className="page-stack product-detail">
      <div className="detail-topbar">
        ...
        <div className="page-header__actions">
          ...
          {item.isActive ? <Button variant="ghost" icon={<Archive />} onClick={() => setArchiveOpen(true)}>Arsipkan</Button> : <StatusBadge tone="neutral">Diarsipkan</StatusBadge>}
        </div>
      </div>
      ...
      <section className="ledger-section">
        ...
        {movements.data?.length ? <div className="ledger-list">{movements.data.map((movement) => (
      ...
      <Modal open={archiveOpen} onOpenChange={setArchiveOpen} title="Arsipkan barang?" description="Barang tidak lagi muncul untuk penjualan baru, tetapi seluruh riwayatnya tetap tersimpan." size="small" footer={<><Button variant="ghost" onClick={() => setArchiveOpen(false)}>Batal</Button><Button variant="danger" pending={archive.isPending} onClick={() => archive.mutate()}>Arsipkan barang</Button></>}>
        <p className="modal-copy">Arsipkan <strong>{item.name}</strong> hanya jika barang ini tidak lagi dijual.</p>
      </Modal>
    </div>
  )
}
